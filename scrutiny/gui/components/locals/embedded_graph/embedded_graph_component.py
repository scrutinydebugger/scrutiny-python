...
# region Acquire Tab
    ...
    def _callback_acquire_request_succeeded(self, request: DataloggingRequest) -> None:
        ...
        def qt_thread_receive_acquisition_data(acquisition: Optional[DataloggingAcquisition], error: Optional[Exception]) -> None:
            if error is not None:
                self._callback_acquire_request_failed(
                    request, error, f"The acquisition succeeded, but downloading its data failed. The content is still available in the server database.\n Error {error}")
                return
