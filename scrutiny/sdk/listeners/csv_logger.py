...
from typing import Optional, TextIO, Dict, Union, List, Iterable
...
class CSVLogger:
    ...
    _watchable_handles:List[WatchableHandle]
    _started:bool
    
    def __init__(self, 
                 folder:str,
                 filename:str,
                 watchable_handles:Optional[List[WatchableHandle]] = None,
        ...
        self._watchable_handles=[]
        if watchable_handles is not None:
            self.set_watchable_handles(watchable_handles)

    def set_watchable_handles(self, watchable_handles:Iterable[WatchableHandle]) -> None:
        if self._started:
            raise RuntimeError("Cannot change the watchable handles list when started")
        
        validation.assert_is_iterable(watchable_handles, 'watchable_handles')
        for handle in watchable_handles:
            validation.assert_type(handle, 'watchable_handles[n]', WatchableHandle)
    ...
    def start(self) -> None:       
        ...
        for i in range(len(self._watchable_handles)):
            if i==0:
                # Safety check to make sure WATCHABLE_FIRST_COL is correct
                assert self.WATCHABLE_FIRST_COL == len(self._fieldnames)
            display_path = self._watchable_handles[i].display_path
            try:
                self._val_dict[display_path] = self._watchable_handles[i].value
                if self._convert_bool_to_int and self._watchable_handles[i].datatype == EmbeddedDataType.boolean:
    ...
    def write(self, updates: List[ValueUpdate]) -> None:
        ...
        self._val_dict[self.DATETIME_HEADER] = datetime.now().strftime(self._datetime_format)
        update_flags = [0]*len(self._watchable_handles)
