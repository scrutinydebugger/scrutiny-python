...
class DeviceHandler:
    ...
    update_ready_event:Optional[threading.Event] # External event that we can set when a call to process() would do something
    ...
    def set_update_ready_event(self, evt:threading.Event) -> None:
        """Assign an event to be set when works is ready to be done and the server can call process()"""
        self.update_ready_event = evt
        self.comm_handler.set_rx_data_event(evt)
