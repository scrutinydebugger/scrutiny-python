...
class TestDatalogging(ScrutinyUnitTest):
    def test_server_acquisition_value_validity(self):
        ...
        axis1 = AxisDefinition(name='axis1', external_id=0)
        axis2 = AxisDefinition(name='axis2', external_id=1)

        acq.add_data(DataSeries([1, 2, 3]), axis1)
        acq.add_data(DataSeries([4, 5, 6]), axis2)

        with self.assertRaises(ValueError):
            acq.add_data(DataSeries([1, 2, 3]), AxisDefinition(name='dup_axis1', external_id=0))
